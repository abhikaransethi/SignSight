...
from preprocessing import detect_hand,image_preprocessing,prediction
...
@app.route("/api/prepare2", methods=["POST"])
def prepare2():
    file = request.files['file']
    res,c, label= preprocessing2(file)
    file_object = io.BytesIO()
    img = Image.fromarray(c.astype('uint8'))
    img.save(file_object, 'PNG')
    base64img = "data:image/png;base64," + b64encode(file_object.getvalue()).decode('ascii')
    ...
    # print(base64img)
    return json.dumps({"image": res.tolist(),"bbox":base64img, "label":label})
@app.route("/api/prepare3", methods=["POST"])
def prepare3():
